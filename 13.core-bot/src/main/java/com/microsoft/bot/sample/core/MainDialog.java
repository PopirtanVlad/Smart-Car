...
import com.microsoft.bot.sample.core.Entities.TurningDetails;
...
public class MainDialog extends ComponentDialog {
    ...
    private final EntityRecognizer luisRecognizer;
     ...
     */
    public MainDialog(EntityRecognizer withLuisRecognizer, BookingDialog bookingDialog) {
        super("MainDialog");
    ...
    private CompletableFuture<DialogTurnResult> actStep(WaterfallStepContext stepContext) {
        ...
        return luisRecognizer.recognize(stepContext.getContext()).thenCompose(luisResult -> {
            switch (luisResult.getTopScoringIntent().intent) {
                ...
                case "turnIntent":
                    ObjectNode directionEntity = luisRecognizer.getDirectionEntities(luisResult);

                    TurningDetails turningDetails = new TurningDetails();
                    turningDetails.setTurnDirection(directionEntity.get("direction").asText());

                    String turnConfirmation="Turning "+turningDetails.getTurnDirection();
                    Activity turnConfimationActivit=MessageFactory.text(turnConfirmation,turnConfirmation,InputHints.IGNORING_INPUT);
                    return stepContext.getContext().sendActivity(turnConfirmation).thenCompose(resourceResponse -> stepContext.next(null));
                default:
